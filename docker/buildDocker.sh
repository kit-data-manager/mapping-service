...
if ! docker build -t ghcr.io/kit-data-manager/"$REPO_NAME":"$TAG_NAME" .; then
  ...
  echo .
  printInfo Now you can create and start the container by calling docker "run -d -p8095:8095 --name mapping4docker ghcr.io/kit-data-manager/$REPO_NAME:$TAG_NAME"
fi
